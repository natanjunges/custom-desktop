...
Pin the Linux Mint repository so it is only used for `chromium` and `firefox` (those Ubuntu packages are transitionals to snaps):
```shell
...
Remove the Linux Mint repository:
```shell
